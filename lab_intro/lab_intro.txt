...
for (double x = 0; x < image.width(); x++) {
    for (double y = 0; y < image.height(); y++) {
      HSLAPixel & pixel = image.getPixel(x, y);
double decrease=1-(sqrt(((static_cast<double>(centerX)-x)*(static_cast<double>(centerX)-x))+(static_cast<double>(centerY)-y)*(static_cast<double>(centerY)-y))*0.005);
if(decrease < 0.2){
decrease=0.2;
}
...
if(secondPixel.l==1.0 && firstPixel.l>0.8){
		firstPixel.l=1.0;
}

	else if(secondPixel.l==1.0){
